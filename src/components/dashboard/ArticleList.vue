<template>
  <DashboardLayout>
    ...
    <div class="db-section-header">
      <div>
        <h1 class="db-section-title">Artigos</h1>
        <p class="db-section-subtitle">Gerencie seus artigos e publicações</p>
      </div>
    ...
    <EmptyState
      ...
      description="Comece criando seu primeiro artigo para compartilhar seu conhecimento."
      action-label="Criar primeiro artigo"
      action-href="/dashboard/artigos/novo"
    />

    <!-- List -->
    <div v-else class="db-list">
      <div v-for="article in articles" :key="article.id" class="db-list-item">
        <img
        ...
        <div class="db-list-actions">
          <a :href="`/artigo/${article.slug}`" target="_blank" class="btn btn-ghost btn-sm" title="Ver publicado">
          ...
          <a :href="`/dashboard/artigos/${article.slug}`" class="btn btn-secondary btn-sm">Editar</a>
          <button class="btn btn-danger btn-sm" @click="deleteTarget = article">
...
import { getArticles, deleteArticle, getFullProfile } from '../../utils/api';
  ...
  try {
    if (activeFilter.value === '') {
      const [drafts, published] = await Promise.all([
        getArticles({ page: 1, limit: 20, status: 'DRAFT', userId }),
        getArticles({ page: 1, limit: 20, status: 'PUBLISHED', userId }),
      ]);
      const merged = [
        ...drafts.data.map(a => ({ ...a, status: 'DRAFT' as const })),
        ...published.data.map(a => ({ ...a, status: 'PUBLISHED' as const })),
      ].sort((a, b) => new Date(b.publishedAt || b.createdAt || 0).getTime() - new Date(a.publishedAt || a.createdAt || 0).getTime());
      const perPage = 15;
      const start = (page.value - 1) * perPage;
      articles.value = merged.slice(start, start + perPage);
      totalPages.value = Math.max(1, Math.ceil((drafts.total + published.total) / perPage));
    } else {
      const res = await getArticles({ page: page.value, limit: 15, status: activeFilter.value, userId });
      articles.value = res.data.map(a => ({ ...a, status: activeFilter.value as 'DRAFT' | 'PUBLISHED' }));
      totalPages.value = res.lastPage;
    }
  } catch {
    toast.value?.show('Erro ao carregar artigos', 'error');
  } finally {
    loading.value = false;
  }
}
