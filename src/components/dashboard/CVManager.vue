<template>
  <DashboardLayout>
    ...
    <Breadcrumb :items="[
      { label: 'Dashboard', href: '/dashboard/candidato' },
      { label: 'Carreira', href: '/dashboard/candidato' },
      { label: 'Currículos' },
    ]" />
    ...
    <div class="db-card" style="margin-bottom: var(--spacing-lg);">
      ...
      <form @submit.prevent="upload" style="display: flex; flex-direction: column; gap: var(--spacing-md); max-width: 480px;">
        ...
        <div class="db-form-group">
          ...
          <input ref="fileInput" data-testid="cv-upload-input" type="file" accept=".pdf" class="db-input" style="padding: 8px;" required @change="onFileChange" />
        </div>
    ...
    <div class="db-card">
      ...
      <div v-else style="display: flex; flex-direction: column; gap: var(--spacing-sm);">
        <div v-for="cv in cvs" :key="cv.id" data-testid="cv-item" class="cv-item">
          <div class="cv-icon">
          ...
          <div style="display: flex; gap: var(--spacing-xs);">
            ...
            >Ativar</button>
            <button class="btn btn-danger btn-sm" data-testid="cv-delete-btn" @click="deleteTarget = cv">
