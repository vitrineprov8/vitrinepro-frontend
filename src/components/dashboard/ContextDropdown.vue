...
const activeContext = computed<ActiveContext>(() => {
  if (pathname.value.startsWith('/dashboard/recrutador/time/')) return 'hunter-time';
  if (pathname.value.startsWith('/dashboard/recrutador/pessoal')) return 'hunter-pessoal';
  return 'profissional';
});

const activeTeamId = computed<string | null>(() => {
  const m = pathname.value.match(/^\/dashboard\/recrutador\/time\/([^/]+)/);
  return m ? m[1]! : null;
});

const activeLabel = computed<string>(() => {
  if (activeContext.value === 'profissional') return 'Candidato';
  if (activeContext.value === 'hunter-pessoal') return 'Recrutador Pessoal';
  if (activeContext.value === 'hunter-time' && activeTeamId.value) {
    const t = teams.value.find((x) => x.id === activeTeamId.value);
    return t ? t.name : 'Recrutador (Time)';
  }
  return 'Recrutador';
});
...
async function navigateTo(
  ...
  try {
    if (context === 'hunter-time' && teamId) {
      ...
      window.location.href = `/dashboard/recrutador/time/${teamId}`;
    } else {
      await profile.setActiveContext(null);
      if (context === 'profissional') {
        window.location.href = '/dashboard/candidato';
      } else {
        window.location.href = '/dashboard/recrutador/pessoal';
      }
    }
  } catch (err) {
