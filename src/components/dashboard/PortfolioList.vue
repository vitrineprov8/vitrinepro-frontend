<template>
  <DashboardLayout>
    ...
    <div v-else class="db-list">
      <div v-for="item in items" :key="item.id" class="db-list-item">
        <img
        ...
        <div class="db-list-actions">
          <a v-if="item.status === 'PUBLISHED'" :href="`/portafolio/${item.slug}`" target="_blank" class="btn btn-ghost btn-sm" title="Ver publicado">
          ...
          <a :href="`/dashboard/portfolio/${item.slug}`" class="btn btn-secondary btn-sm">Editar</a>
          <button class="btn btn-danger btn-sm" @click="deleteTarget = item">
...
import { getPortfolioItems, deletePortfolioItem, getFullProfile } from '../../utils/api';
